...
(function (SB) {
    ...
    const viaMcp = function () {
      ...
      return chain
        .then(function (urls) {
          ...
          /* One url. An array was ACCEPTED and then ignored — the first
             picture came through and the subject of the second was invented
             — which is the one failure a fallback cannot catch, because
             nothing errors. Several references travel as one sheet instead. */
          const args = {
            org_id: orgId(),
            prompt: canon.prompt,
  ...
  /* ---------------- the reference sheet ----------------
   *
   * generate_image honours ONE picture. Not one because it refuses more —
   * it accepted an array of three and used the first, inventing the subject
   * the second one was of — which is the failure mode that cannot be caught
   * from the outside. So the several become one.
   *
   * Four at most: past that each face is too small to hold an identity, and
   * the advice limit already says most models start averaging references
   * rather than reading them.
   */
  const SHEET_MAX = 4;
  const SHEET_PLAN = {
    2: { cols: 2, rows: 1, names: ['left', 'right'] },
    3: { cols: 2, rows: 2, names: ['top-left', 'top-right', 'bottom-left'] },
    4: { cols: 2, rows: 2, names: ['top-left', 'top-right', 'bottom-left', 'bottom-right'] }
  };
  ...
  function sheetPlan(p, shot, role) {
    ...
    return list.map(function (e, i) {
      return { n: i + 1, label: e.label, role: e.role || '', panel: plan.names[i], e: e };
    });
  }
  ...
  /* The sheet itself: each reference at the best size the file holds, letter-
   * boxed rather than cropped — a face cropped to fit a square is a different
   * face — on a flat grey ground so an empty corner reads as nothing. */
  function buildSheet(p, cells) {
    const plan = SHEET_PLAN[cells.length];
    if (!plan || !canDraw()) return Promise.resolve(null);
    const CELL = 768;
    const cv = document.createElement('canvas');
    cv.width = plan.cols * CELL;
    cv.height = plan.rows * CELL;
    ...
    return Promise.all(cells.map(function (c, i) {
      const e = c.e;
      const src = (e.render && SB.Renders.dataUrl(p, e.render)) ||
        (e.img ? SB.Blobs.src(p, e.img) : '');
      return src ? load(src).then(function (im) { return { im: im, i: i }; })
        : Promise.resolve({ im: null, i: i });
    })).then(function (loaded) {
      let drew = 0;
      loaded.forEach(function (x) {
        if (!x.im || !x.im.naturalWidth) return;
        drew++;
        const col = x.i % plan.cols, row = Math.floor(x.i / plan.cols);
        const r = Math.min(CELL / x.im.naturalWidth, CELL / x.im.naturalHeight);
        const w = Math.round(x.im.naturalWidth * r), h = Math.round(x.im.naturalHeight * r);
        ctx.drawImage(x.im, col * CELL + (CELL - w) / 2, row * CELL + (CELL - h) / 2, w, h);
      });
      if (!drew) return null;
      return new Promise(function (resolve) {
        if (cv.toBlob) cv.toBlob(function (b) { resolve(b); }, 'image/png');
        else dataUrlToBlob(cv.toDataURL('image/png')).then(resolve, function () { resolve(null); });
      });
    }).catch(function () { return null; });
  }
  ...
  function refBlobs(p, shot) {
    ...
    /* More than one, and they travel as one picture — because one picture is
       all that is honoured, and the alternative is the second subject being
       invented. */
    const plan = sheetPlan(p, shot, 'image');
    if (plan.length > 1) {
      return buildSheet(p, plan).then(function (b) {
        if (b) {
          return [{ blob: b, name: 'refsheet.png', label: 'reference sheet', n: 1,
            sheet: plan.map(function (c) {
              return { n: c.n, label: c.label, role: c.role, panel: c.panel };
            }) }];
        }
        return single(p, list);       // no canvas: the first, as before
      });
    }
    return single(p, list);
  }

  /* One picture: the only one the call will carry. Uploading the others pays
  ...
  function refsFor(p, shot, role) {
    ...
    const plan = sheetPlan(p, shot, 'image');
    const sheet = plan.length > 1;
    const carries = transport() === 'key' ? 0
      : (sheet ? plan.length : Math.min(1, feed.length));
    return {
      role: 'image',
      feed: feed.length,
      wordsOnly: wordsOnly,
  ...
  SB.Imagine = {
    ...
    shootKey: shootKey, takesRefs: takesRefs, arrivalRefs: arrivalRefs,
    sheetPlan: sheetPlan, _buildSheet: buildSheet, report: report, door: door,
